...
async function loadFontMetrics(fontUrl: string, char: string, fontSizePx: number) {
  ...
  if (isWoff2) {
    ...
    const typoAsc = os2?.sTypoAscender ?? (font as any).tables?.hhea?.ascender ?? font.ascender ?? 0;
    const typoDesc = os2?.sTypoDescender ?? (font as any).tables?.hhea?.descender ?? font.descender ?? 0;
...
export default function MetricGlyphPreview({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const yCap = metrics.capHeight != null ? paddingPx + metrics.ascent - metrics.capHeight : null;
    ...
    const HLine = (y: number, title: string, color: string) => (
      <g key={title}>
        ...
        <text
          ...
          textAnchor="end"
          fontFamily="whisper"
          className="text-sm text-white font-whisper"
        >
          {title}
        </text>
      </g>
  ...
  if (!state.ready) {
    return (
      <div className={['w-full aspect-[3/2] grid place-items-center', className].join(' ')}>
        <span className="text-sm font-whisper text-white">Loading font metrics…</span>
      </div>
    );
  }
