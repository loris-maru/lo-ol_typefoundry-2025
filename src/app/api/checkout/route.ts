...
export async function POST(req: NextRequest) {
  try {
    ...
    const byTypeface: Record<string, TypefaceRow> = Object.fromEntries(
      typefaces.map((t) => [t.name, t]),
    );
    ...
    for (const item of parsed.items) {
      ...
      if (!typeface) {
        console.error(
          `Unknown typeface ${item.fontFamilyId}. Available typefaces:`,
          Object.keys(byTypeface),
        );
      ...
      const amount = priceForItem(
        basePrice,
        item.licenseType as LicenseType,
        item.userTier as UserTier,
      );
      console.log(
        `Item pricing: basePrice=${basePrice}, license=${item.licenseType}, tier=${item.userTier}, amount=${amount}`,
      );
    ...
    if (err instanceof z.ZodError) {
      ...
      return NextResponse.json(
        { error: "Invalid cart payload", details: err.issues },
        { status: 400 },
      );
    }
